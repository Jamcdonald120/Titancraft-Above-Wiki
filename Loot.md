---
title: Loot
description: What kind of stuff can you get from The Above?
published: true
date: 2026-08-10T15:17:16.843Z
tags: 
editor: markdown
dateCreated: 2026-08-08T00:49:58.903Z
---

# Loot

## Titanshards 
### Splinters

The most frequent loot in the Above is the Titanshard Splinter. These cannot be redeemed on their own and must be converted to Titanshard Gems first. [Morgana](npcs/Morgana), the cleric of the Shrine of the Shattered Shard, will convert 9 Splinters into 1 Gem for you. Wandering Traders may offer a better exchange rate.

### Gems

Titanshard Gems are the highly sought after currency on Titancraft. They can be exchanged for a random reward at the Well in the Village, or exchanged for specific items with various traders around the Above.


## The well

In the Village is the well. The well allows players to turn their Titanshard Gems into exclusive loot. To use the well, spin the arrow to point to the category of item you want, throw in the corresponding amount of gems, and pull the lever! Specific categories of items require 5 Gems be thrown into the well, while fully random requires only 3

### Weapons

- Mace - Bane of mortals
- Head Hunter Axe

### Tools

- Boomstick (correct category?)

### Armor

- Leggings - Leap 2 enchantment

### Coupons

- Armor Stand: Trade to a Moderator for one (1) custom armor stand. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Banner: Use at a Coupon Kiosk to create a custom banner with more patterns than normally possible. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Block: Trade to a Moderator for one (1) aesthetic creative-only block, placed wherever you wish. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Book/Sign: Trade to a Moderator for one (1) custom sign or written book with formatting and/or URL links. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- DisplayL Trade to a Moderator for one (1) text, item, or block display, placed wherever you wish. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Edible item - New for Season 11: Use in an anvil to make an item (or stack of items) edible. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Enchantment: Use in an anvil to give an item (or stack of items) a shiny enchantment glint. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Hat: Use in an anvil to give an item the ability to be equipped to the head slot. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Item Frame: Trade to a Moderator for one (1) custom item frame. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Lore: Use at a Coupon Kiosk to add custom lore text to an item of your choice. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Mannequin - New for Season 11: Trade to a Moderator for one (1) custom mannequin. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Miniblock: Use at a Coupon Kiosk to receive a customized miniblock to use for decoration. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Name: Use at a Coupon Kisok to rename an item using custom formatting and colors. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Scale: Use at a Coupon Kisok to set the desired scale. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Silence: Use on a mob to silence it. Visit the Kiosk area or type "/coupons-info" for details.
- Sound: Use at a Coupon Kiosk to receive a customized sound to use on noteblocks. Some conditions apply; visit the Kiosk area or type "/coupons-info" for details.
- Spawner: Right-click an entity with this coupon to set decorative spawner type. Place the coupon into a crafting grid to receive a spawner.


### Resources

- Every pottery sherd

### Potion
 - Extra suspicious stew (correct category?)
 - Night vision
 
 > This section is very incomplete, please add things you've had from the well! {.is-info}